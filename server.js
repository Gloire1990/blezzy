...
// =============================================================
// ✅ DATABASE CONNECTION (Preserving all previous users)
// =============================================================
...
mongoose.connect(MONGO_URI)
    .then(() => console.log("✅ Database Connected - Records Intact"))
    .catch(err => console.error("❌ Database Error:", err));

// =============================================================
// ✅ DEPLOYMENT CONFIG (Render & Cloudflare optimized)
// =============================================================
...
// =============================================================
// ✅ DATA SCHEMA (Matching your previous structure)
// =============================================================
...
// =============================================================
// ✅ AUTHENTICATION (Admin Email: swedbank.bolia@icloud.com)
// =============================================================
...
app.post('/login', async (req, res) => {
    ...
    if (user && user.passcode === inputPass) {
        ...
        // Ensure the specific admin email always has admin rights
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (existing) return res.send("User exists. <a href='/'>Login</a>");
        
        const isAdmin = email.toLowerCase() === "swedbank.bolia@icloud.com";
        const newUser = new User({
            name, email: email.toLowerCase(), phone, address, passcode, isAdmin
        });
        await newUser.save();
        res.redirect('/');
...
// =============================================================
// ✅ BUSINESS LOGIC (Swap, Deposit, Admin)
// =============================================================
...
app.get('/admin', async (req, res) => {
    ...
    const pending = await User.find({ "pendingDeposit.status": "Pending" });
    res.send(renderAdmin(pending));
});
...
// =============================================================
// ✅ UI COMPONENTS (Previous Mobile-First Style)
// =============================================================
...
function renderDashboard(u){
    ...
    u.investments.forEach(inv => {
        const diffDays = Math.ceil(Math.abs(now - inv.date)/(1000*60*60*24));
        if(diffDays >= 30) claimable += (inv.capital + inv.profit);
    ...
    <div class="container">
        <div class="balance-card">
            <small style="opacity:0.7">Portfolio Balance</small>
            <div style="font-size:32px; font-weight:800; margin:10px 0">$${u.balance.toFixed(2)}</div>
            <div style="font-size:12px">Locked: $${locked.toFixed(2)}</div>
        </div>
        <div class="action-grid">
            <div class="action-btn" onclick="openM('dep')"><div class="icon-circle"><i class="fa-solid fa-plus"></i></div>Deposit</div>
            <div class="action-btn" onclick="alert('Withdrawals process on Fridays.')"><div class="icon-circle"><i class="fa-solid fa-wallet"></i></div>Withdraw</div>
            <div class="action-btn" onclick="document.getElementById('swp').submit()"><div class="icon-circle" style="${claimable > 0 ? 'background:#00c853' : ''}"><i class="fa-solid fa-unlock"></i></div>Claim</div>
            <div class="action-btn" onclick="openM('cnt')"><div class="icon-circle"><i class="fa-solid fa-headset"></i></div>Support</div>
        </div>
        <form id="swp" action="/swap" method="POST" style="display:none"></form>
        <h3>Transactions</h3>
        ${u.transactions.slice(-5).reverse().map(t => `<div class="card"><small>${t.date}</small><div>${t.type}: $${t.amount}</div></div>`).join('') || '<p style="color:#444">No history yet</p>'}
        <br><a href="/logout" style="color:#444; font-size:12px">Logout Session</a>
    </div>
    <div id="dep" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); z-index:100; align-items:center; justify-content:center; padding:20px;">
        <div style="background:#11141a; width:100%; padding:25px; border-radius:20px;">
            <h2>Deposit USD</h2>
            <form action="/dep" method="POST"><input type="number" name="amount" placeholder="Amount" required><button>Continue</button></form>
            <button onclick="this.parentElement.parentElement.style.display='none'" style="background:none">Cancel</button>
        </div>
    </div>
    <script>function openM(id){document.getElementById('dep').style.display='flex'}</script>
    </body></html>`;
}
...
function renderAdmin(pending){ 
    ...
    <h2>Admin Center</h2>
    ${pending.map(p=>`<div class="card"><b>${p.name}</b><br>$${p.pendingDeposit.amount}<form action="/confirm" method="POST"><input type="hidden" name="uid" value="${p._id}"><button style="background:#00c853">Confirm Deposit</button></form></div>`).join('') || '<p>No pending tasks</p>'}
    <a href="/dashboard" style="color:#194bfd">Personal Dashboard</a>
    </div></body></html>`; 
}
